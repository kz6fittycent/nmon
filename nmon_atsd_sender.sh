...
hostname="`hostname`"
nmonfile="$dir/${hostname}_${starttime}.nmon"
user="atsdreadonly"
...
server=$1
shift  1
while getopts "hs:c:m:u:i:p:r:" opt
do
    case $opt in
        h) 
...
NMONFILECONST="${hostname}_150216_1124.nmon"
    ...
    if [ "$listen" = "" -o "$sh" = "" ]; then
        ...
        if [ "$tl" != "" -a "$tn" != "" ]; then
            ...
            if [ "$reconnectDelayMultiply" -gt "10" ]; then
                writeLog "reconnect number: $reconnectDelayMultiply, killing telnet, tail, ssh."
                killproc $telnetpid "telnet"
                killproc $tailpid "tail"
                killproc $sshpid "ssh"
                return 2
...
if $CHKKE; then
    ...
    if [ "`ls -la $keypath | awk '{print $1}' | cut -c 2`" != "r" -o "`ls -la $keypath | awk '{print $1}' | cut -c 5-10`" != "------" ]; then
        writeLog "private ssh-key ( $keypath ) permission is `ls -la $keypath | awk '{print $1}'`. Key shoud be available for reading by user, but not accessible by group or others."
...
writeLog "start working with senderPID: $$, $0 $server $@, FILENAME: $nmonfile"
defineFreeLocalport
